 ...
 *      
 */
int exec_remote_cmd_loop(char *ip, int port) {
    int sock;
    ...
    while (1) {
        ...
        if (strlen(cmd_line) == 0 || strspn(cmd_line, " \t\n") == strlen(cmd_line)) {
            // Send empty command to get warning
            if (send(sock, "\n", 1, 0) < 0) {
                perror("send");
                break;
            }
            ...
            while ((bytes_read = recv(sock, io_buff, RDSH_COMM_BUFF_SZ - 1, 0)) > 0) {
                if (io_buff[bytes_read - 1] == RDSH_EOF_CHAR) {
                    bytes_read--;
        ...
        if (send(sock, cmd_line, strlen(cmd_line), 0) < 0) {
            perror("send");
            break;
        }

        // Read response from server until EOF
        while ((bytes_read = recv(sock, io_buff, RDSH_COMM_BUFF_SZ - 1, 0)) > 0) {
            // Check for EOF character
            if (io_buff[bytes_read - 1] == RDSH_EOF_CHAR) {
                bytes_read--;  // Don't print the EOF character
                if (bytes_read > 0) {
                    printf("%.*s", (int)bytes_read, io_buff);
                }
                break;
            }
            printf("%.*s", (int)bytes_read, io_buff);
        ...
        // Check for exit or stop-server commands
        if (strcmp(cmd_line, "exit") == 0 || strcmp(cmd_line, "stop-server") == 0) {
            break;
        }
    }
