...
int open_db(char *dbFile, bool should_truncate)
{
    ...
    fd = open(dbFile, flags, perms);
    if (fd < 0) {
        printf(M_ERR_DB_OPEN);
        return ERR_DB_FILE;
    }
    
    off_t size = MAX_STD_ID;
    size *= STUDENT_RECORD_SIZE;
...
int get_student(int fd, int id, student_t *s)
{
    ...
    bytes = read(fd, s, STUDENT_RECORD_SIZE);
    if(bytes < 0) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    
    if(bytes != STUDENT_RECORD_SIZE || !s->id)
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    new.id = id;
    new.gpa = gpa;
    strncpy(new.fname, fname, 23);
    new.fname[23] = 0;
    strncpy(new.lname, lname, 31);
    new.lname[31] = 0;
    
    spot = id - 1;
    spot *= STUDENT_RECORD_SIZE;
    ...
    rc = write(fd, &new, STUDENT_RECORD_SIZE);
...
int del_student(int fd, int id)
{
    ...
    found = get_student(fd, id, &kid);
    if(found != NO_ERROR) {
        printf(M_STD_NOT_FND_MSG, id);
        return ERR_DB_OP;
    }
    
    where = STUDENT_RECORD_SIZE;
    where *= (id - 1);
    ...
    kid = EMPTY_STUDENT_RECORD;
    if(write(fd, &kid, STUDENT_RECORD_SIZE) < STUDENT_RECORD_SIZE) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
int count_db_records(int fd)
{
    ...
    for(;;) {
        got = read(fd, &buf, STUDENT_RECORD_SIZE);
        if(got == 0) 
            break;
        if(got < STUDENT_RECORD_SIZE) {
            printf(M_ERR_DB_READ);
            return ERR_DB_FILE;
        }
        if(buf.id)
...
int print_db(int fd)
{
    ...
    while(1) {
        bytes = read(fd, &kid, STUDENT_RECORD_SIZE);
        if(!bytes)
            break;
        if(bytes < 0) {
            printf(M_ERR_DB_READ);
        ...
        if(kid.id) {
            if(!found) {
                printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST_NAME", 
                       "LAST_NAME", "GPA");
                found = 1;
            }
            real_gpa = kid.gpa / 100.0;
            printf(STUDENT_PRINT_FMT_STRING, kid.id, kid.fname, 
                   kid.lname, real_gpa);
        }
    }
...
int compress_db(int fd)
{
    ...
    newfd = open(TMP_DB_FILE, O_RDWR|O_CREAT|O_TRUNC, 0660);
    if(newfd < 0) {
        printf(M_ERR_DB_OPEN);
        return ERR_DB_FILE;
    }
    ...
    while((n = read(fd, &buf, STUDENT_RECORD_SIZE)) > 0) {
        if(buf.id != 0) {
            if(write(newfd, &buf, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE)
                goto write_err;
            next += STUDENT_RECORD_SIZE;
        }
    }
    ...
    if(ftruncate(newfd, MAX_STD_ID * STUDENT_RECORD_SIZE) < 0)
        goto write_err;
